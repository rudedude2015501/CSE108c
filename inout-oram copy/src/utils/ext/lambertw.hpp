/*
  ...
  Adapted from: https://github.com/DarkoVeberic/LambertW
...
inline double LambertWSeries(const double p) {
  ...
  if (ap < 0.01159) {
    ...
    return -1 +
           p * (1 + p * (q[2] +
                         p * (q[3] +
                              p * (q[4] + p * (q[5] + p * (q[6] + p * (q[7] + p * (q[8] + p * (q[9] + p * q[10]))))))))
               );
  } else {
    return -1 +
           p * (1 +
               ...
               z * (1.5 -
                    z * (2.6666666666666666667 -
                         z * (5.2083333333333333333 -
                              z * (10.8 - z * (23.343055555555555556 -
...
template <typename Float, double IterationStep(const Float x, const Float w)> struct Iterator {
...
template <typename Float> struct Pade<Float, 0, 1> {
  static inline Float Approximation(const Float x) {
    return x *
           HORNER4(
               Float, x, 0.07066247420543414, 2.4326814530577687, 6.39672835731526, 4.663365025836821, 0.99999908757381
           ) /
...
template <typename Float> struct Pade<Float, 0, 2> {
  static inline Float Approximation(const Float x) {
    ...
    return 2 + y *
                   HORNER3(
                       Float, y, 0.00006979269679670452, 0.017110368846615806, 0.19338607770900237, 0.6666648896499793
                   ) /
...
template <typename Float> struct Pade<Float, -1, 4> {
  static inline Float Approximation(const Float x) {
    return HORNER4(
               Float, x, -2793.4565508841197, -1987.3632221106518, 385.7992853617571, 277.2362778379572,
               -7.840776922133643
           ) /
...
template <typename Float> struct Pade<Float, -1, 6> {
  static inline Float Approximation(const Float x) {
    ...
    return -exp(
        HORNER4(
            Float, y, 0.000026987243254533254, -0.007692106448267341, 0.28793461719300206, -1.5267058884647018,
            -0.5370669268991288
        ) /
...
template <typename Float> struct Pade<Float, -1, 7> {
  static inline Float Approximation(const Float x) {
    return -1 -
           sqrt(
               HORNER4(
                   Float, x, 988.0070769375508, 1619.8111957356814, 989.2017745708083, 266.9332506485452,
                   26.875022558546036
               ) /
...
template <int branch> inline double LambertW(const double x);

template <> inline double LambertW<0>(const double x) {
  typedef double d;
  return Y5(
      (Branch<d, 0>::BranchPointExpansion<8>(x)), x < -0.367679,
      (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Branch<d, 0>::BranchPointExpansion<10>(x))), x < -0.311,
      (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Pade<d, 0, 1>::Approximation(x))), x < 1.38,
      (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Pade<d, 0, 2>::Approximation(x))), x < 236,
      (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Branch<d, 0>::AsymptoticExpansion<6 - 1>(x)))
  );
}

template <> inline double LambertW<-1>(const double x) {
  typedef double d;
  return Y7(
      (Branch<d, -1>::BranchPointExpansion<8>(x)), x < -0.367579,
      (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Branch<d, -1>::BranchPointExpansion<4>(x))), x < -0.366079,
      (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Pade<d, -1, 7>::Approximation(x))), x < -0.289379,
      (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Pade<d, -1, 4>::Approximation(x))), x < -0.0509,
      (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Pade<d, -1, 5>::Approximation(x))), x < -0.000131826,
      (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Pade<d, -1, 6>::Approximation(x))), x < -6.30957e-31,
